    ...
    try {
      ...
      if (typeof j.id === "string" && j.status) {
        setImages((prev) =>
          prev.map((row) =>
            row.id === imageId
              ? { ...row, featureRequest: { id: j.id!, status: j.status! } }
              : row,
          ),
        );
      }
    ...
    <div className={className ? `${className} space-y-14` : "space-y-14"}>
      <section aria-labelledby="partner-images-heading" className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Public images readers created on your published titles. Request featuring on the Discover community strip.
          </p>
        </div>
        {images.length === 0 ? (
          <p className="text-sm text-text-muted">No public images on your books yet.</p>
        ) : (
          <>
      ...
      <section aria-labelledby="partner-requests-heading" className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Track requests you have submitted for admin review ({ownFeatureRequests.length} shown).
          </p>
        </div>
        {ownFeatureRequests.length === 0 ? (
          <p className="text-sm text-text-muted">No feature requests yet.</p>
        ) : (
