    ...
    >
      {books.map((book) => {
        ...
        const hoverCardClass =
          shelvedUnavailable
            ...
            : isRow
              ...
              : "hover:scale-[1.02] hover:border-accent/60 hover:shadow-lg hover:shadow-bg-overlay/15";
        const hoverLinkClass =
          shelvedUnavailable || isRow ? "" : "hover:scale-[1.01] sm:hover:scale-[1.02]";

        const cover = (
          <div
            className={`relative shrink-0 overflow-hidden bg-bg-surface ${
          ...
          >
            ...
            {shelvedUnavailable ? (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-bg-base/75 px-1.5 py-2 backdrop-blur-[1px]">
                <p className="text-center text-[9px] font-semibold uppercase leading-snug tracking-wide text-text-primary sm:text-[10px]">
        ...
        const details = (
          ...
          >
            <h2
              className={`font-serif font-semibold leading-snug text-text-primary ${
                !shelvedUnavailable
            ...
            {book.genre ? (
              <span className="mt-0.5 inline-flex w-fit max-w-full truncate rounded border border-accent/35 bg-accent-muted/80 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-accent-text/90">
        ...
        return (
          <li key={book.id} className={isRow ? "" : "h-full"}>
            <div
              className={`flex overflow-hidden rounded-lg border border-border/95 bg-bg-surface shadow-sm shadow-bg-overlay/5 transition duration-200 ${hoverCardClass} ${
                shelvedUnavailable ? "opacity-90" : ""
              } ${
                isRow
                  ? "flex-row items-stretch"
                  : `h-full flex-col shadow-md shadow-bg-overlay/10`
              }`}
            >
              {!shelvedUnavailable ? (
