...
Your public gallery images and comments are handled slightly differently: we remove your username and any other identifying attribution from them, but the content itself may remain visible to other users rather than being deleted outright. Once that identifying link is removed, it's no longer tied to you. This applies in addition to the content-removal rules below, anything that was already a confirmed policy violation comes down regardless.

**Content removed for policy violations.** Separately from account-level retention, any specific image, question, or comment that we confirm violates our Acceptable Use Policy is removed from public view as soon as that's confirmed, regardless of whether the associated account is active, suspended, or terminated. Content that hasn't been flagged isn't affected by another piece of content on the same account being removed. We keep an internal record of confirmed violations for enforcement history even after the content itself is taken down.

Content involving apparent child sexual abuse material is handled as a distinct, urgent process: immediate removal from anywhere it could be accessed, followed by reporting to the appropriate authorities as required by law, which may include preserving a copy for investigators for a legally required period before final destruction.
...
3. Section 6's content-retention question is now fully resolved for both branches: voluntary deletion de-identifies and keeps public content live, suspension/termination removes only confirmed violations and otherwise leaves the account and its content untouched. The remaining work here is engineering, not policy: the self-deletion flow needs to actually strip attribution from public images/comments rather than deleting the rows outright, which is a different operation than what it does today (full wipe). Worth a short Cursor prompt for this whenever you're ready.
3a. The CSAM takedown-and-reporting process referenced in Section 6 needs an actual written runbook, reviewed by a lawyer, covering exactly what to report, to whom, how quickly, and how long any copy must be preserved before destruction. This is materially more urgent than most other open items here, it's an active legal obligation, not a business preference, and shouldn't stay as a one-paragraph policy sentence for long. The runbook needs to explicitly cover attempted-but-blocked generation, not just confirmed material: NCMEC tracks "attempted generation via text prompts" as its own reporting category, separate from confirmed AI-generated CSAM, and industry practice (e.g. OpenAI's stated policy) is to report attempts through the same channel, not just successful generations. The practical difference is what gets preserved and reported, there's no image file in the blocked-attempt case, just the prompt text, account identifier, and timestamp, which is a much lighter preservation burden than confirmed material, but it still needs to actually go out as a report, not just sit in an internal moderation log. Confirm with the lawyer exactly what minimum information needs to be included.
