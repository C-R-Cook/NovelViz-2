...
NovelViz's three core legal documents (Privacy Policy, Terms of Service, Acceptable Use Policy) were drafted in full during June 2026, working through actual product behavior rather than generic templates. They are not yet live on the site. Current versions are maintained outside this repo for now; recommend copying them into the repo (e.g. `/docs/legal/`) once finalized so they version alongside the code they describe, the way this reference doc does.

**Status:** drafted, not yet published. Each still has a `[DATE]` placeholder for "Last updated"/"Effective date," and the Terms has a placeholder liability cap figure, both need real values filled in before going live. The Acceptable Use Policy's CSAM reporting section needs lawyer review before publishing, this is a live legal obligation, not just a wording question, see below.

### Minimum age: 18, not 13

Eligibility is **18 and over**, full stop, not the 13+ general-audience standard most consumer apps use. This was a deliberate choice to ship something simple now rather than build out the protections a 13-17 cohort would actually require (parental consent flows, data minimization for minors, age-appropriate design considerations under newer US state laws). The classroom/school use case that originally prompted this discussion is **paused, not implemented**, no institutional consent flow exists, and none should be built ad hoc, any future school deployment needs a deliberate arrangement, not an individual teacher signup.

Engineering implication: the **`Under 18` option must be removed from the age range dropdown** (`lib/age-range.ts`, `USER_AGE_RANGE_OPTIONS`) wherever it still appears in onboarding/account profile pickers, since it lets a true minor casually self-report their age after signup, creating actual knowledge of underage use that the eligibility policy is trying to avoid. An explicit age affirmation ("I confirm I am 18 or older") at signup, self-reported and unverified, is the standard mechanism, not a guarantee, just the trigger point that makes the policy's "we ask users to represent their age" language true. (Being handled directly by Chris, outside the Cursor-prompt workflow, as of this version.)

### Legal consent at sign-up (supersedes parts of `legal-consent-signup.md`)

The original three-separate-checkbox, ask-twice design described in `legal-consent-signup.md` has been simplified:

- **Two checkboxes, not three**, at `/register`: "I am 18 years of age or older" and a combined "I agree to the Terms of Service and Privacy Policy." The combined checkbox still writes two separate DB timestamps (`termsAcceptedAt`, `privacyAcceptedAt`) from one click, the UI consolidated, the audit trail didn't.
- **Bridge-cookie pattern** (mirroring the existing `onboarding_plan_done` cookie): checking the boxes at `/register` sets a short-lived intent cookie; `/auth/consent` auto-submits the authoritative DB write the moment provisioning finishes if that cookie is present, fresh, and matches the current document-version constants, rather than showing the same checkboxes a second time. Falls back to the original explicit form if the bridge is missing, stale, or a document version changed mid-flow.
- **Implemented** (done outside the Cursor-prompt workflow for this round; `legal-consent-signup.md` itself has not yet been updated to reflect this and should get a pass once there's a free cycle, same idea as keeping this reference doc current).
- The Acceptable Use Policy is **incorporated by reference into the Terms of Service**, not a fourth mandatory checkbox, deliberately, to avoid re-opening the consent flow that was just simplified.

### Account status, suspension, and termination

The three legal documents now assume a three-state account model that **does not fully exist yet** and is the subject of active Phase 1 investigation (see Open Items): `active`, `suspended`, `terminated`. Key rules the eventual implementation needs to satisfy:

- **Suspension is provisional.** Credit balance and subscription status are left completely untouched while suspended, pending an appeal. A suspended user can still sign in, but lands on a restricted status page (not the normal app), showing their actual strike/flag history (not an abstract policy summary) and a free-text explanation form that creates an appeal record and emails admin via the existing `lib/admin-email.ts` pattern (new `AdminEmailCategory`, e.g. `ACCOUNT-APPEAL`).
- **Termination is final.** Triggered either by a denied appeal or a zero-tolerance violation that skips suspension entirely. Unused credits are forfeited via a new immutable ledger entry (`CreditTransaction` with a negative amount, new `CreditTransactionReason` value, e.g. `FORFEITED_TERMINATION`), never by mutating or deleting rows, consistent with the existing "balance is always `SUM(CreditTransaction.amount)`" principle. A terminated user signing in sees a final notice page, no explanation form, since their chance to explain was at the suspension stage (or never existed, for zero-tolerance cases).
- **Termination must never delete the `User` row or the Clerk identity.** This is the opposite of voluntary self-deletion, and is easy to get backwards by analogy. The entire point of retaining a terminated account's record is to keep enforcement history accurate and to prevent the same email being used to register a fresh, unbanned account, Clerk itself is what blocks duplicate-email signups, but only while the original identity still exists.
- **The existing self-deletion flow must refuse to run for `suspended` or `terminated` accounts.** Without this guard, a banned user could simply delete their own account immediately after being flagged, erasing the moderation record and freeing their email for a fresh signup, defeating the point above entirely.

Full implementation spec: see the Cursor prompt covering Phase 1 (investigate current state) and Phase 2 (implement). Phase 1 results pending as of this version, see Open Items.

### Voluntary account deletion vs. termination (now resolved, was an open question for a while)

- **Voluntary self-deletion**: personal account data is wiped immediately (DB row removed, Clerk identity removed), exactly as before. **Public gallery images and comments are de-identified rather than deleted**, attribution/username stripped, content itself may remain visible. This is a change from the current full-wipe behavior, current behavior deletes everything; Cursor has already flagged this exact mismatch between the new Terms/Privacy language and current code, correctly. Needs its own Cursor prompt (not yet written as of this version).
- **Banned/terminated accounts**: covered above, data is retained, not deleted, the opposite of self-deletion.
- **Content takedown is a separate axis from account-level retention.** Any specific image, question, or comment confirmed to violate the Acceptable Use Policy is removed from public view as soon as that's confirmed, regardless of whether the account is active, suspended, or terminated. Content that was never flagged isn't affected by other content on the same account being removed. Internal `ModerationLog` records are the permanent evidence trail; there's no need to leave violating content publicly visible "as evidence."

### Credits

- **Purchased credits never expire**, with two exceptions: voluntary account deletion and account termination both forfeit any unused balance, no refund, since both result in the account's data/access being given up (one by choice, one as a consequence).
- The account deletion flow needs an explicit warning screen showing the user's current credit balance and stating plainly that deleting is permanent and forfeits it, before they can confirm. Not yet built, the actual deletion sentence in the Terms currently describes UI that doesn't exist yet.
- Dormant-account auto-deletion threshold is set at **2 years** of inactivity, but should not be automatically enforced until there's a way to send a warning email first (depends on Loops being live). Until then, treat it as a stated policy without automated enforcement.

### CSAM handling (the highest-stakes item in this section)

- The Acceptable Use Policy's zero-tolerance rule covers **"create, request, or attempt to create"**, not just successful generation. This matters because attempted-but-blocked generation is itself a distinct, real-world reporting category, not a gray area: NCMEC tracked over 30,000 reports of attempted CSAM generation via text prompts in 2025 alone, separate from confirmed AI-generated material, and major platforms (e.g. OpenAI's stated policy) report attempts through the same channel as confirmed material.
- Practical difference: a blocked attempt has no image artifact to preserve, just prompt text, account identifier, and timestamp, a much lighter preservation burden than confirmed material, but it still needs to actually be reported, not just logged internally.
- **A written, lawyer-reviewed incident response runbook is still needed**, covering exactly what to report, to whom (Cybertip.ca in Canada, NCMEC-equivalent for US users), how fast, and how long any copy must be preserved before destruction, for both confirmed material and blocked attempts. This is an active legal obligation, not a product preference, and is the single most time-sensitive item anywhere in this section.
- Separately, there's an open question (also for the lawyer) about personal liability exposure given NovelViz is technically the API customer transmitting prompts to Anthropic and fal.ai. The relevant legal concept is "knowing" conduct, criminal CSAM statutes generally require knowing production/distribution, not strict liability for a good-faith platform with a real moderation system where something nonetheless got through. Section 230 does **not** apply to federal criminal law, so it isn't a backstop here. Confirm NovelViz is incorporated and that vendor accounts sit under the business entity rather than personally, as a separate, more general protective step.

### Cookies, analytics, and affiliate links

- No cookie consent banner is currently needed (only essential auth cookies + first-party analytics, both exempt from consent requirements in most regimes including EU/UK ePrivacy rules). The trigger point for needing one is adding any third-party analytics or ad pixel, treat that as a tripwire, not something to build preemptively.
- Amazon affiliate links (planned) are disclosed in the Privacy Policy's cookies/third-party-links section. Separately, an FTC-style affiliate disclosure near the links themselves (not just in the privacy policy) is still needed when that feature ships, this is a distinct requirement from privacy law.

### Governing law

Terms of Service uses a simple British Columbia courts / BC law clause, deliberately **no arbitration clause**. fal.ai's AAA/mass-arbitration apparatus (used as a reference point while drafting) is built for a US company's litigation exposure at scale and doesn't transplant well onto a Canadian solo-founder business. Revisit only if a lawyer specifically recommends it once there's real revenue and volume.

### Document files

| Document | Status |
|---|---|
| `NovelViz-Privacy-Policy.md` | Drafted, internal notes intact, not yet published |
| `NovelViz-Terms-of-Service.md` | Drafted, internal notes intact, not yet published |
| `NovelViz-Acceptable-Use-Policy.md` | Drafted, internal notes intact, not yet published |

All three reference each other by section number where relevant (e.g. the Acceptable Use Policy points to the Terms' Partner and Author Content section); cross-references are internally consistent with each other as of this version, but **not yet consistent with whatever is currently live on `/terms` and `/privacy` today**, since those pages haven't been replaced with this drafted content yet. When that publish step happens, the live pages and these documents become the same thing and the mismatch resolves itself; until then, treat the drafted `.md` files as the source of truth for what the rules actually are, not the live pages.
...
- Account status model (`active`/`suspended`/`terminated`) and credit forfeiture ledger logic — Phase 1 investigation underway (Cursor prompt run in Plan mode), results pending. Phase 2 implementation, including the suspended/terminated status pages and the explanation-form-to-admin-email flow, depends on Phase 1's findings.
- Account deletion flow update — not yet a written Cursor prompt as of this version. Two changes bundled: (1) credit-forfeiture warning screen before confirming deletion, (2) change underlying logic from full-wipe to de-identify-and-keep for public gallery content. Cursor has already correctly flagged the mismatch between current full-wipe behavior and the new Terms/Privacy language.
- Publishing the three legal documents live: building the `/acceptable-use` page (doesn't exist yet), replacing `/terms` and `/privacy` content with the finalized drafts, bumping `TERMS_DOCUMENT_VERSION`/`PRIVACY_DOCUMENT_VERSION` in `lib/legal-consent.ts` to match.
- Remaining placeholders needing real decisions before publishing: liability cap dollar figure (Terms), "Last updated"/"Effective date" on all three documents.
- CSAM incident-response runbook — lawyer review needed, not an engineering task. See Legal & Compliance section above for what it needs to cover.
- `legal-consent-signup.md` describes the old ask-twice consent design; it's now out of date relative to the bridge-cookie implementation (done outside the Cursor-prompt workflow) and should get an update pass.
- Several places in this document still describe a "beta" pricing/access model (Onboarding Plan Picker, Pricing model notes). The Terms of Service has deliberately dropped beta framing in favor of a different launch model. These references haven't been reconciled throughout this document yet, flagging so they aren't mistaken for current behavior.
