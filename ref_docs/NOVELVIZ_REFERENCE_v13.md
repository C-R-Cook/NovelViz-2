# NovelViz Reference
**Version 13** — Updated June 2026 — Removed beta phase; launching directly to Free / Standard / Premium / Partner
...
| Images | Cloudinary — single shared account for dev + prod, paths env-prefixed (`novelviz/dev/…` / `novelviz/prod/…`, auto-detected via `VERCEL_ENV`). Covers split by source: `covers/user/[bookId]` (manual/EPUB/Open Library) vs `covers/ai/[bookId]` (Cover AI commit). Gallery images at `gallery/[imageId]`. 2:3 crop fit for covers. See **Cloudinary Environment Separation** below. |
...
CLOUDINARY_URL
NOVELVIZ_CLOUDINARY_ENV             # Optional override: "dev" or "prod" — normally auto-detected from VERCEL_ENV
...
- Neon consumption API requires Launch plan or above. Currently on free tier — upgrade before launch.

---

## Database Environments

Use **separate Postgres databases** for production and local development. `DATABASE_URL` is read everywhere (Prisma app, CLI scripts, migrations). `DIRECT_URL` is read by `prisma.config.ts` for migrations and some scripts when set.

**Two Neon branches:**
- `production` (or `main`) — live site data
- `development` — local dev, experiments, `prisma db seed`, Gutenberg CLI tests

Create dev from prod (one-time): Neon console → Branches → Create branch → parent `production` → name `development`. Copy each branch's pooled (→ `DATABASE_URL`) and direct (→ `DIRECT_URL`) connection strings.

**Vercel env vars by environment** (set per-environment via the checkboxes in Vercel project settings — Production / Preview / Development):
- **Production:** production Neon branch (pooled `DATABASE_URL` + direct `DIRECT_URL`), Clerk production keys. Never attach the dev Neon branch here.
- **Preview:** no single fixed rule — options are (A) a small dedicated Neon preview branch, (B) leave DB vars unset and accept preview builds won't hit a DB, or (C) point Preview at the **development** branch. Never point Preview at production.
- **Development (Vercel CLI):** development Neon branch, Clerk **development** instance keys — used when running `vercel env pull`.
...
- Production migrations run automatically via `scripts/vercel-build.sh` on Vercel production deploys (build command should be `sh scripts/vercel-build.sh`, or left cleared to use the repo's `vercel.json` default — never paste the shell one-liner without its leading `if`)
...
npx prisma db seed
```
Seed creates `dev_user_admin`, readers, partners, sample books. A real Clerk user gets a DB row on first sign-in against whichever DB `DATABASE_URL` points at — sign in locally only after the dev DB is configured.
...
**Copying prod → dev (optional, rare):** prefer a Neon branch reset from parent, or logical dump/restore. Avoid copying prod users' PII to dev unless required — usually seed + fresh ingest on dev is enough.

### Cloudinary Environment Separation

Dev and production **share one Cloudinary account** — separation is by folder path, not by account. Paths are env-prefixed and auto-detected:
...
Auto-detection logic lives in `lib/cloudinary.ts`. `NOVELVIZ_CLOUDINARY_ENV` (`dev` or `prod`) overrides detection — leave unset for normal Vercel/local use.

**Covers are now split by source** rather than living in one folder: `covers/user/[bookId]` for manually-sourced covers (EPUB extraction, Open Library, manual partner/admin upload) vs `covers/ai/[bookId]` for committed Cover AI generations. This replaces the old single `novelviz/covers/[bookId]` path — see updated **Cloudinary Folders** table under Cover AI.

### One-off Data Migration Scripts

Two one-time scripts support the dev→prod cutover; not part of normal day-to-day workflow.

**Reorganize existing Cloudinary assets** (flat legacy paths → env-prefixed structure above). Run against whichever DB the asset records live in:
```bash
...
**Promote dev catalogue → production.** Copies all books owned by `dev_user_admin` on the development Neon branch to production — chapters, embeddings, and Cloudinary covers re-uploaded under `novelviz/prod/…`. Default target owner: `cmpgiq4wj000004k361w2uwz7`.
```bash
...
**Recommended order:** deploy app → `promote-catalogue-dev-to-prod --apply` → `cloudinary-reorganize-assets --apply` (only if legacy flat-path URLs remain on prod after promotion).
...
**Role vs. Tier distinction:** `UserRole` (reader / partner / admin) controls platform access type. `SubscriptionTier` (free / standard / premium) controls AI action allowances. These are independent. Admins bypass quota checks entirely regardless of tier. Partners default to the Free tier on signup (a free reader account is included with every Partner account) and may separately subscribe to Standard or Premium for their own reading.
...
Three reader subscription tiers — **Free**, **Standard**, and **Premium** — each with a monthly allowance of Q&A queries and image generations, plus a **Partner** account type for publishers/authors (includes a free reader account by default). Limits are enforced in real-time. When the monthly allowance is exhausted, users may draw from a persistent **credit balance** purchased separately.
...
3. Resolve billing period via `resolveBillingPeriod()`.
4. Count monthly usage rows since `periodStart`.
5. Get credit balance (`SUM` of `CreditTransaction.amount`).
6. Decision: `limit === null` (unlimited) → allowed; `used < limit` → allowed (quota); `creditBalance >= creditCost` → allowed (credits); otherwise → **not allowed**.
...
- Four cards: Free, Standard, Premium, Partner.
...
- Partner card sets **Free** tier (a Partner account includes a free reader account by default) and creates a `PartnerRequest` row (deduped). Partner role approval is a separate admin process. Partners may separately subscribe to Standard or Premium for their own reading.
- **Stripe checkout must be wired to the plan picker before launch** — selecting Standard or Premium should route into Stripe Checkout with card collection at signup; Free and Partner skip Checkout and write the tier directly.
...
2. Cover extracted → Cloudinary (`novelviz/{env}/covers/user/[bookId]`)
...
All paths are env-prefixed — see **Cloudinary Environment Separation** under Database Environments for the `{env}` auto-detection logic (`dev` / `prod` via `VERCEL_ENV`, overridable with `NOVELVIZ_CLOUDINARY_ENV`).

| Folder | Contents |
|---|---|
| `novelviz/{env}/covers/user/[bookId]` | Manually-sourced covers — EPUB extraction, Open Library, manual partner/admin upload |
| `novelviz/{env}/covers/ai/[bookId]` | Official committed Cover AI covers |
| `novelviz/{env}/cover-drafts/[bookId]/[uuid]` | Temporary Cover AI draft previews |
| `novelviz/{env}/gallery/[imageId]` | Library Imagine generated images |

**Note:** covers were previously a single `novelviz/covers/[bookId]` folder regardless of source. The user/ai split was introduced alongside the env-prefix change; legacy flat-path assets are migrated via `scripts/cloudinary-reorganize-assets.ts` (see Database Environments).
...
- Pre-launch billing: wire Stripe checkout to plan picker. Standard/Premium selections route into Checkout with card collection at signup; Free and Partner write the tier directly. No beta phase — this must be complete before going live.
...
- Cloudinary dev/prod separation — ✅ DONE (v12: env-prefixed paths `novelviz/dev/…` / `novelviz/prod/…` auto-detected via `VERCEL_ENV`, covers split into `covers/user` vs `covers/ai`; one-off `cloudinary-reorganize-assets.ts` + `promote-catalogue-dev-to-prod.ts` scripts added for the cutover)
...
- Cover AI draft cleanup is best-effort — orphaned drafts in `novelviz/{env}/cover-drafts/` possible if modal closed mid-generate.
- Credit pack reader storefront is minimal — users reach credit purchase via Stripe portal.
- Stripe checkout not yet connected to onboarding plan picker — must be wired before launch.
...
- `CURSOR.md` + `NOVELVIZ_REFERENCE_v13.md` maintained as persistent context
...
- Reference doc is now `NOVELVIZ_REFERENCE_v13.md` — update `CURSOR.md` to point at the new version
