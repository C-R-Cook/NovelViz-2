...
| Method | Route | Purpose |
|---|---|---|
| GET | `/api/books/[bookId]/cover-ai/config` | Models, quota fields, `hasPendingQuotaRequest` |
| POST | `/api/books/[bookId]/cover-ai/generate` | Generate draft; `maxDuration = 300` |
| POST | `/api/books/[bookId]/cover-ai/commit` | Publish chosen draft, destroy all drafts |
| POST | `/api/books/[bookId]/cover-ai/discard-drafts` | Cleanup on modal close |
| POST | `/api/books/[bookId]/cover-ai/request-more` | Partner quota exhaustion request |
...
- AI-assisted prompt generation — send book context to Claude or OpenAI, partner tweaks the result before generating
- Model choice for partners (currently fixed to Seedream v4.5)
...
- Custom Clerk auth — branded login page + post-register plan picker (Free / Standard / Premium columns + Partner tab). Beta pricing: Standard price crossed out with "Free during beta" beneath; Premium visible but faded with "Coming soon". Note on plan picker: after beta, accounts downgrade to free tier, users can then choose to upgrade.
