...
See the **Authentication** section for the full onboarding flow and stage routing logic.

- Four cards: Free, Standard, Premium (Coming Soon — not selectable), Partner.
- Plan data fetched dynamically from DB via `getPublicTierPlans()` — no hardcoded limits on the page. Credit pack prices come from the first active `CreditPack` row.
- On selection, calls `completePlanStep()` server action (`lib/onboarding-plan-action.ts`): writes `subscriptionTier` to `User`, calls `establishLimitFloorsForTier()`, sets `onboarding_plan_done` cookie, redirects to `/onboarding/preferences`.
- Partner card: `completePlanStep({ tier: "standard", partnerInterest: true })` — sets Standard tier, creates a `PartnerRequest` row (deduped per user), then continues to preferences like a normal selection. Partner role approval happens outside this flow (admin process).
- **Beta behaviour:** during beta all accounts use Standard features at no charge. After beta, accounts revert to Free tier unless the user upgrades. No card details are collected at sign-up during beta.
...
| `lib/session-profile.ts` | `getOnboardingStage()`, `getOnboardingRedirectUrl()`, `getPostAuthRedirectUrl()` |
...
| `app/api/onboarding/route.ts` | Profile completion (username, genres, optional fields); `409` if username taken |
| `app/api/onboarding/check-username/route.ts` | Public username availability check (`GET ?username=...`) |
| `app/api/webhooks/clerk/route.ts` | Clerk `user.created` webhook → DB user provisioning |
...
| `app/auth/after/auth-after-provisioning.tsx` | DB provisioning poll screen |
| `components/clerk-themed-auth.tsx` | Themed Clerk sign-up/in widget |
...
- Pre-beta-end billing ramp: collect card details and wire Stripe checkout to plan picker before beta ends, so users can opt in to paid tiers at cutover. No card collection during beta itself.
...
- Stripe checkout not yet connected to onboarding plan picker — tier is set directly in DB during beta. Must be wired before beta ends so paid tiers activate cleanly at cutover.
- Post-beta revert: all beta accounts drop to Free tier at beta end. No card details collected at sign-up. Users must actively upgrade to regain Standard/Premium features.
...
- `CURSOR.md` + `NOVELVIZ_REFERENCE_v6.md` maintained as persistent context
